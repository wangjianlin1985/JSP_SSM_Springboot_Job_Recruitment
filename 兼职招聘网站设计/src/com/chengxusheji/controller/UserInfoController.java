...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,String name,String birthDate,String xl,String telephone,String shzt,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthDate, xl, telephone, shzt, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthDate, xl, telephone, shzt);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,String name,String birthDate,String xl,String telephone,String shzt,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (telephone == null) telephone = "";
		if (shzt == null) shzt = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthDate, xl, telephone, shzt, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthDate, xl, telephone, shzt);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void update(@Validated UserInfo userInfo, BindingResult br,
		...
		String jianliFileFileName = this.handleFileUpload(request, "jianliFileFile");
		if(!jianliFileFileName.equals(""))userInfo.setJianliFile(jianliFileFileName);
		String smrzzlFileName = this.handleFileUpload(request, "smrzzlFile");
		if(!smrzzlFileName.equals(""))userInfo.setSmrzzl(smrzzlFileName);
		try {
	...
	public void OutToExcel(String user_name,String name,String birthDate,String xl,String telephone,String shzt, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(xl == null) xl = "";
        if(telephone == null) telephone = "";
        if(shzt == null) shzt = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,name,birthDate,xl,telephone,shzt);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","出生日期","学生照片","学历","联系电话","邮箱","审核状态","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getGender(),userInfo.getBirthDate(),userInfo.getUserPhoto(),userInfo.getXl(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getShzt(),userInfo.getRegTime()});
        }
        /*
