...
public class JobRecordController extends BaseController {
	...
	public void userAdd(JobRecord jobRecord, BindingResult br,
		...
		jobRecord.setUserObj(userObj);
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		jobRecord.setRecordDate(sdf.format(new java.util.Date()));
	...
	public String userFrontlist(String title,@ModelAttribute("userObj") UserInfo userObj,String recordDate,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (recordDate == null) recordDate = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "JobRecord/jobRecord_userFrontquery_result"; 
	}
