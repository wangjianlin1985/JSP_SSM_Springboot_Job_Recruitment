...
public class CompanyService {
	...
	public boolean checkLogin(Admin admin) throws Exception { 
		Company db_company = (Company) companyMapper.getCompany(admin.getUsername());
		if(db_company == null) { 
			this.errMessage = " 账号不存在 ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_company.getPassword().equals(admin.getPassword())) {
			this.errMessage = " 密码不正确! ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_company.getShzt().equals("已审核")) {
			this.errMessage = " 审核中! ";
			System.out.print(this.errMessage);
			return false;
		}
