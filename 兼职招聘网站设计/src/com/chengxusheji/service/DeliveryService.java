...
public class DeliveryService {
    ...
    /*按照查询条件分页查询简历投递记录*/
    public ArrayList<Delivery> companyQueryDelivery(String companyUserName,UserInfo userObj,String deliveryTime,String handleTime,String clzt,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(!companyUserName.equals(""))  where += " and t_company.companyUserName='" +  companyUserName +"'";
    	if(null != userObj &&  userObj.getUser_name() != null  && !userObj.getUser_name().equals(""))  where += " and t_delivery.userObj='" + userObj.getUser_name() + "'";
    	if(!deliveryTime.equals("")) where = where + " and t_delivery.deliveryTime like '%" + deliveryTime + "%'";
    	if(!handleTime.equals("")) where = where + " and t_delivery.handleTime like '%" + handleTime + "%'";
    	if(!clzt.equals("")) where = where + " and t_delivery.clzt like '%" + clzt + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void companyQueryTotalPageAndRecordNumber(String companyUserName,UserInfo userObj,String deliveryTime,String handleTime,String clzt) throws Exception {
     	String where = "where 1=1";
     	if(!companyUserName.equals(""))  where += " and t_company.companyUserName='" +  companyUserName +"'";
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_delivery.userObj='" + userObj.getUser_name() + "'";
    	if(!deliveryTime.equals("")) where = where + " and t_delivery.deliveryTime like '%" + deliveryTime + "%'";
